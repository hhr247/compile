...
void Lexical_analyzer::Solution(char* fname,Symbol_table* Symbol) {
	char* token;
	...
	while (true) {
		...
		flag = 1;
		if (state == -1) {
			printf("Line %d has ERROR\n", line);
			outfile << "Line " << line << " has ERROR \n";
			break;
		}
		if (state == 1) {
			continue;
			...
			token[i++] = '\0';
			string str = token;
			int code = state_to_code(state_before, token, Symbol);
			if (code == 4) {
				int pos = Symbol->add_master(str);
				outfile << "< " << pos << ", " << code << ", " << line_before << ">\n";
			...
			//	token[0] = '\"';
			//	token[1] = '\0';
			//	int code1 = state_to_code(4, token, Symbol);
			...
			if (code) {
				...
				outfile << "< notes, " << code << ", " << line_before << ">\n";
			}
			//if (code == 3) {
			//	int code1 = state_to_code(4, token, Symbol);
			//	outfile << "< " << token << ", " << code1 << "," << line_before << ">\n";
...
bool Lexical_analyzer::is_letter(char a) {
	return (a >= 'a' && a <= 'z') || (a >= 'A' && a <= 'Z');
}

bool Lexical_analyzer::is_dig(char a) {
	return (a >= '0' && a <= '9');
}
...
int Lexical_analyzer::state_to_code(int state_before, char* token, Symbol_table* Symbol) {
	if (state_before == 2) {
		string str = token;
		if (KT.count(str)) {
			return KT[str];
...
void Lexical_analyzer::reset(FILE*& fp, char*& token, int& state, int& i) {
	i = 0;
	state = 1;
	if(!feof(fp)) fseek(fp, -1L, 1);
}